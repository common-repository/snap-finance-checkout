...
function snap_finance_load_textdomain() {
	...
	if ( $woocommerce_snap_finance_settings ) {
		if ( $woocommerce_snap_finance_settings['title'] != 'Snap Finance' || $woocommerce_snap_finance_settings['description'] != 'Credit-challenged? Snap approves $300 up to $5,000 in accessible lease-to-own financing.' ) {
			$woocommerce_snap_finance_settings['title']       = 'Snap Finance';
			$woocommerce_snap_finance_settings['description'] = 'Credit-challenged? Snap approves $300 up to $5,000 in accessible lease-to-own financing.';
...
function wc_snap_finance_style() {
	...
	if ( $payment_method ) {
		if ( $payment_method == 'snap_finance' ) {
			...
			$order_data_items   = $wpdb->get_results($wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . "woocommerce_order_items WHERE order_item_type = 'line_item' AND order_id =%d", $order_id ));
			...
			$shape              = 'pill';
						...
						// $wc_obj = WC();
						// $cart_fees_mj = WC()->cart->get_fees();
						// $order_data_mj = WC()->order->get_data();
			...
			if ( $cart_totals['fee_total'] != '' && $cart_totals['fee_total'] != '0' ) {
				// Add the total of all fees as another item in order for Snap Payload
				$item_data     = '';
				$item_qty      = '1';
				$single_price  = $cart_totals['fee_total'];
				$product_id    = '0';
				$sku           = '1';
				$order_items[] = array(
					'price'       => strval( $single_price ),
...
function snap_get_checkout_order_received_url( $order_id, $order_key ) {
	...
	return apply_filters( 'snap_woocommerce_get_checkout_order_received_url', $order_received_url, $order_id );
}
		...
		if ( $payment_method == 'snap_finance' ) {
			if ( $snap_order ) {
				...
				<p class="form-field form-field-wide wc-customer-user">
					...
					<input type="text" value="<?php echo esc_attr($order_deliveryDate); ?>" name="order_deliveryDate"
