...
function set_order_deliveryDate( $order_id ) {
	$order_deliveryDate=sanitize_text_field($_POST['order_deliveryDate']);
	if ( isset( $_POST['order_deliveryDate'] ) && preg_match('/^[0-9]{4}-[0-1][0-9]-[0-3][0-9]$/',$order_deliveryDate)) {
		update_post_meta( $order_id, '_order_deliveryDate', $order_deliveryDate );
	}
}
...
function snap_finance_complete_order( $order_id = '' ) {
	...
	if ( isset( $_REQUEST['deliver_date'] ) ) {
		$deliveryDate = sanitize_text_field($_REQUEST['deliver_date']);
	} else {
		$deliveryDate = sanitize_text_field($_POST['order_deliveryDate']);
	}

	$payment_method = get_post_meta( $order_id, '_payment_method', true );
	$snap_order     = get_post_meta( $order_id, '_snap_order', true );
	if ( $payment_method && ! empty( $deliveryDate ) && $snap_order && preg_match('/^[0-9]{4}-[0-1][0-9]-[0-3][0-9]$/',$deliveryDate)) {
		if ( $payment_method == 'snap_finance' ) {
			$application_id       = get_post_meta( $order_id, '_applicationId', true );
			$snap_finance_setting = get_option( 'woocommerce_snap_finance_settings' );
			$snap_finance_token   = get_snap_finance_token();
			...
			if ( $snap_finance_setting['snap_finance_mode'] == 'sandbox' ) {
				$api_url = constant( 'SANDBOX_COMPLETE_API_URL' ) . $application_id . '/status';
			} else {
				$api_url = constant( 'LIVE_COMPLETE_API_URL' ) . $application_id . '/status';
			}
